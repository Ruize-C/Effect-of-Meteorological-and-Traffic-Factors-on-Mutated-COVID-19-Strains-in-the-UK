...
city <- read.csv("National average _1.csv", header=TRUE, fileEncoding="cp936")
...
pred.temp <- crosspred(cb.Temperature, virus_model, cen=virus[which.min(virus$R0alp), 2], bylag=0.2)
pred.humi <- crosspred(cb.Humidity, virus_model, cen=virus[which.min(virus$R0alp), 3], bylag=0.2)
pred.wind <- crosspred(cb.WindSpeed, virus_model, cen=virus[which.min(virus$R0alp), 4], bylag=0.2)
pred.UV <- crosspred(cb.UV, virus_model, cen=virus[which.min(virus$R0alp), 11], bylag=0.2)
pred.tarrival <- crosspred(cb.tarrival, virus_model, cen=virus[which.min(virus$R0alp), 12], bylag=0.2)

png(file = "alp_tempreature.png", width = 3000, height = 2000, res = 300)
...
png(file = "alp_humidity.png", width = 3000, height = 2000, res = 300)
...
png(file = "alp_windspeed.png", width = 3000, height = 2000, res = 300)
par(mar=c(4,5,3,5),oma=c(2,2,2,2))
...
png(file = "alp_UV.png", width = 3000, height = 2000, res = 300)
par(mar=c(4,5,3,5),oma=c(2,2,2,2))
...
png(file = "alp_arrival.png", width = 3000, height = 2000, res = 300)
par(mar=c(4,5,3,5),oma=c(2,2,2,2))
