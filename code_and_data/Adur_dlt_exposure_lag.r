...
city <- read.csv("National average _2.csv", header=TRUE, fileEncoding="cp936")
...
pred.temp <- crosspred(cb.Temperature, virus_model, cen=virus[which.min(virus$R0dlt), 2], bylag=0.2)
pred.humi <- crosspred(cb.Humidity, virus_model, cen=virus[which.min(virus$R0dlt), 3], bylag=0.2)
pred.wind <- crosspred(cb.WindSpeed, virus_model, cen=virus[which.min(virus$R0dlt), 4], bylag=0.2)
pred.UV <- crosspred(cb.UV, virus_model, cen=virus[which.min(virus$R0dlt), 11], bylag=0.2)
pred.tarrival <- crosspred(cb.tarrival, virus_model, cen=virus[which.min(virus$R0dlt), 12], bylag=0.2)

png(file = "dlt_tempreature.png", width = 3000, height = 2000, res = 300)
...
png(file = "dlt_humidity.png", width = 3000, height = 2000, res = 300)
...
png(file = "dlt_windspeed.png", width = 3000, height = 2000, res = 300)
par(mar=c(4,5,3,5),oma=c(2,2,2,2))
...
png(file = "dlt_UV.png", width = 3000, height = 2000, res = 300)
par(mar=c(4,5,3,5),oma=c(2,2,2,2))
...
png(file = "dlt_arrival.png", width = 3000, height = 2000, res = 300)
par(mar=c(4,5,3,5),oma=c(2,2,2,2))
